...
from AnanyaxMusic.utils.database import add_sudo, remove_sudo
...
@akash.on_message(filters.command(["addsudo"]) & filters.user(OWNER_ID))
@language
...
@akash.on_message(filters.command(["delsudo", "rmsudo"]) & filters.user(OWNER_ID))
@language
...
@akash.on_message(filters.command(["sudolist", "listsudo", "sudoers"]) & ~BANNED_USERS)
@language
async def sudoers_list(client, message: Message, _):
    text = _["sudo_5"]
    user = await akash.get_users(OWNER_ID)
