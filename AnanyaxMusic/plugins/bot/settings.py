...
from AnanyaxMusic.utils.database import (
    add_nonadmin_chat,
    get_authuser,
    get_authuser_names,
    get_playmode,
...
from AnanyaxMusic.utils.decorators.admins import ActualAdminCB
from AnanyaxMusic.utils.decorators.language import language, languageCB
from AnanyaxMusic.utils.inline.settings import (
    auth_users_markup,
    playmode_users_markup,
    setting_markup,
    vote_mode_markup,
)
from AnanyaxMusic.utils.inline.start import private_panel
...
async def settings_mar(client, message: Message, _):
    ...
    await message.reply_text(
        _["setting_1"].format(akash.mention, message.chat.id, message.chat.title),
        reply_markup=InlineKeyboardMarkup(buttons),
    )


@akash.on_callback_query(filters.regex("settings_helper") & ~BANNED_USERS)
@languageCB
...
@akash.on_callback_query(filters.regex("settingsback_helper") & ~BANNED_USERS)
@languageCB
async def settings_back_markup(client, CallbackQuery: CallbackQuery, _):
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        await akash.resolve_peer(OWNER_ID)
        OWNER = OWNER_ID
        buttons = private_panel(_)
        return await CallbackQuery.edit_message_text(
            _["start_2"].format(CallbackQuery.from_user.mention, akash.mention),
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    else:
        buttons = setting_markup(_)
...
@akash.on_message(filters.command(["repo", "source"]) & filters.private)
async def send_repo(_, message: Message):
    await message.reply_video(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("🎵 Music + Management Bot Repo", url="https://github.com/ZoxxOP/AnanyaMusic")],
                [InlineKeyboardButton("🎧 Only Music Bot Repo", url="https://github.com/ZoxxOP/AnanyaxMusic")]
            ]
        ),
...
@akash.on_callback_query(filters.regex("FERRARIUDTI") & ~BANNED_USERS)
@ActualAdminCB
...
@akash.on_callback_query(filters.regex(pattern=r"^(AUTH|AUTHLIST)$") & ~BANNED_USERS)
@ActualAdminCB
    ...
    if command == "AUTHLIST":
        ...
        if not _authusers:
            ...
            for note in _authusers:
                ...
                try:
                    user = await akash.get_users(user_id)
...
@akash.on_callback_query(filters.regex("VOMODECHANGE") & ~BANNED_USERS)
@ActualAdminCB
