...
async def executor(client: akash, message: Message):
...
@akash.on_callback_query(filters.regex(r"runtime"))
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@akash.on_callback_query(filters.regex("forceclose"))
...
async def maintenance_executor(client: akash, message: Message):
    if len(message.command) < 2:
...
async def maintenance_shellrunner(_, message: Message):
    if len(message.command) < 2:
        return await edit_or_reply(message, text="<b>ᴇxᴀᴍᴩʟᴇ :</b>\n/sh git pull")
    text = message.text.split(None, 1)[1]
    if "\n" in text:
